...
export default function MainPaneTopBar({ content }: { content: FileType }) {
    ...
    return content && (
        <nav aria-label="top-bar" className="flex flex-col z-10">
            {/* top half */}
            <div className="flex justify-between bg-[#252526] h-[36px]">
                ...
                <div className="flex bg-[#1e1e1e] justify-center items-center pl-4 pr-[6px] *:mx-1 relative cursor-pointer">
                    ...
                    <div onClick={clearContent} className="group relative flex justify-center hover:bg-[#ffffff15] p-[2px] rounded-sm">
                        ...
                        <span className="hidden group-hover:flex absolute w-min whitespace-nowrap top-[22.5px] px-2 py-[2px] border-1 border-[#ffffff30] bg-[#252526] rounded-sm text-[13.5px] normalText">Close (Esc)</span>
                    </div>
                </div>
            ...
            <div className="flex items-center bg-[#1e1e1e] h-[26px] shadow-[0_4px_4px_-2px_rgba(0,0,0,0.1)] shadow-black normalText text-[15px]">
                ...
                <div className={path ? "flex items-center pl-1 *:mr-2 group cursor-pointer" : "flex items-center pl-[24px] *:mr-2 group cursor-pointer"}>
                    {icon}
                    <span className="group-hover:text-[#ffffffdb]">{filename}</span>
                </div>
            </div>
        </nav>
    );
